...
public class BattleCharacter : MonoBehaviour {
    ...
    public void UpdateSilenceDuration()
    {
        if (silenced && silenceDuration > 0)
        {
            silenceDuration--;
            if (silenceDuration <= 0)
            {
                ...
                if (silenceUi != null)
                {
                    silenceUi.SetActive(false);  // Hide the silence UI indicator when cured
                }
                
                Debug.Log(characterName + " is no longer silenced!");
            }
        }
    }
}
