...
public class BattleManager : MonoBehaviour
{
    ...
    //For using skill on player characters
    ...
    public void NextTurn()
    {
        ...
        // **New Code: Update Silence Duration**
        activeBattlers[currentTurn].UpdateSilenceDuration();
    ...
    public void EnemyAttack()
    {
        ...
        {
            // Randomly choose a skill to use
            int selectAttack = Random.Range(0, activeBattlers[currentTurn].skills.Length);
            BattleSkill skill = activeBattlers[currentTurn].skills[selectAttack].skill;

            // If the skill is meant to heal status effects, heal the enemy instead of dealing damage
            if (skill.healStatusEffects)
            {
                // Heal the enemy's status effects
                activeBattlers[currentTurn].poisoned = false;
                activeBattlers[currentTurn].silenced = false;

                // Display notification
                battlePrompts.notificationText.text = activeBattlers[currentTurn].characterName + " used " + skill.skillName + " and cured its own status effects!";
                battlePrompts.Activate();

                // Show visual effects for the skill, if available
                if (skill.effect != null)
                {
                    Instantiate(skill.effect, activeBattlers[currentTurn].transform.position, activeBattlers[currentTurn].transform.rotation);
                }
            }
            else
            {
                // If not a healing skill, use normal attack logic
                ...
                // Deal damage to a random player target
                DealDamage(selectedTarget, skill.skillPower, skill);
            }
    ...
    public void DealDamage(int target, int movePower, BattleSkill skill)
    {
        if (skill.healStatusEffects)
        {
            // If healing status effects, cure both poison and silence
            activeBattlers[target].poisoned = false;
            activeBattlers[target].silenced = false;
            activeBattlers[target].silenceDuration = 0;  // Reset silence duration

            Debug.Log(activeBattlers[currentTurn].characterName + " used " + skill.skillName + " to cure status effects on " + activeBattlers[target].characterName);

            battlePrompts.notificationText.text = activeBattlers[currentTurn].characterName + " used " + skill.skillName + " and cured status effects";
            battlePrompts.Activate();

            // Show skill effect, if available
            if (skill.effect != null)
            {
                Instantiate(skill.effect, activeBattlers[target].transform.position, activeBattlers[target].transform.rotation);
            }
        }
        else
        {
            ...
            if (!skill.strengthModifier && !skill.defenseModifier)
            {
                ...
                if (skill.canSilence)
                {
                    ...
                    activeBattlers[target].silenceDuration = skill.duration; // Apply silence duration

                    if (!activeBattlers[target].character)
                    {
                        activeBattlers[target].silenceUi.SetActive(true); // Show the silence UI indicator
                    }

                    Debug.Log(activeBattlers[currentTurn].characterName + " used " + skill.skillName + " and inflicted silence for " + skill.duration + " turns on " + activeBattlers[target].characterName);

                    battlePrompts.notificationText.text = activeBattlers[currentTurn].characterName + " used " + skill.skillName + " and inflicted silence for " + skill.duration + " turns";
                    battlePrompts.Activate();
                }
        ...
        if (skillMenu.activeInHierarchy)
        {
            ...
            if (!healStatusEffect)
            {
                if (activeBattlers[currentTurn].currentSP >= selectedSkillCost && activeBattlers[currentTurn].currentHp > 0)
                {
                    ...
                    //Revive
                    else if (activeBattlers[selectChar].currentHp == 0 && revive)
                    {
                        activeBattlers[selectChar].currentHp += selectedSkillPower;
                    }
